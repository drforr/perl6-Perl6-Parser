...
for ( True, False ) -> $*PURE-PERL {
	subtest {
		...
		subtest {
			...
			subtest {
				...
				$source = Q{sub foo( ) { }};
				$tree = $pp.to-tree( $source );
				is $pp.to-string( $tree ), $source, Q{intra-term ws};
			}, Q{empty};
	...
	subtest {
		...
		$tree = $pp.to-tree( $source );
		is $pp.to-string( $tree ), $source, Q{christmas tree, minimal spacing};
	...
	_END_
		$tree = $pp.to-tree( $source );
		is $pp.to-string( $tree ), $source, Q{christmas tree, maximal spacing};
	}, Q{christmas tree};
	...
	_END_
	$tree = $pp.to-tree( $source );
	is $pp.to-string( $tree ), $source, Q{separate function name and open paren};
}
